...
def generate_and_store_boxscores(conn, start_season, end_season):
    """Gathers team boxscores and adds tables to database"""
    add_basic_boxscores(conn, start_season, end_season, if_exists='append')
    add_advanced_boxscores(conn, start_season, end_season, if_exists='append')
    add_scoring_boxscores(conn, start_season, end_season, if_exists='append')
    return None


def generate_and_store_betting_data(conn, start_season, end_season):
    """Scrapes historical spreads and moneylines from sportsbookreview.com"""
    add_spreads(conn, start_season, end_season, if_exists='append')
    add_moneylines(conn, start_season, end_season, if_exists='append')
    return None
