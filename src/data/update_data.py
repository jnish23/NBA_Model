...
def update_team_gamelogs_1H(season= '2020-21'):
    """
    Updates the basic team gamelogs csv file for the given season
    INPUTS:
    season (str): current season which you will update ie: '2020-21'
    ...
    dfs = []
    for season_type in ['Regular Season', 'Playoffs']:
        h1_boxscores = teamgamelogs.TeamGameLogs(season_nullable=season, game_segment_nullable='First Half',
                                                 season_type_nullable=season_type).get_data_frames()[0]
        dfs.append(h1_boxscores)
        
    path = '../data/basic_team_boxscores_1H/team_boxscores_1H_{}.csv'.format(season)
    team_gamelogs_1H = pd.concat(dfs)
    team_gamelogs_1H.to_csv(path, index=False)

    return None



def update_scoring_boxscores_1H(season):
    # Pull the GAME_IDs from my data
    current_player_boxscores = pd.read_csv('../data/scoring_player_boxscores_1H/player_scoring_1H_{}.csv'
                                           .format(season), dtype={'GAME_ID':'object'})
    current_team_boxscores = pd.read_csv('../data/scoring_team_boxscores_1H/team_scoring_1H_{}.csv'
                                         .format(season), dtype={'GAME_ID':'object'})
    ...
    missing_game_ids = set(to_date_game_ids) - set(current_game_ids)
    num_games_updated = len(missing_game_ids)
    print("num_scoring_boxscores_updated:", num_games_updated)
   
    if num_games_updated == 0:
        print("Scoring Boxscores_1H All up to date")
        return None
    ...
    for game_id in tqdm(missing_game_ids, desc='progress'):
        boxscores = boxscorescoringv2.BoxScoreScoringV2(game_id, 
                                                          range_type='1', 
                                                          start_period='0',
                                                          end_period='2',
                                                          timeout=200).get_data_frames()
    ...
    updated_player_boxscores.to_csv('../data/scoring_player_boxscores_1H/player_scoring_1H_{}.csv'
                                    .format(season), index=False)
    updated_team_boxscores.to_csv('../data/scoring_team_boxscores_1H/team_scoring_1H_{}.csv'
                                  .format(season), index=False)


    return None

def update_advanced_boxscores_1H(season):
    # Pull the GAME_IDs from my data
    current_player_boxscores = pd.read_csv('../data/advanced_player_boxscores_1H/player_advanced_1H_{}.csv'
                                           .format(season), dtype={'GAME_ID':'object'})
    current_team_boxscores = pd.read_csv('../data/advanced_team_boxscores_1H/team_advanced_1H_{}.csv'
                                         .format(season), dtype={'GAME_ID':'object'})
    ...
    if num_games_updated == 0:
        print("Advanced Boxscores_1H All up to date")
        return None
    ...
    for game_id in tqdm(missing_game_ids, desc='progress'):
        boxscores = boxscoreadvancedv2.BoxScoreAdvancedV2(game_id, 
                                                          range_type='1', 
                                                          start_period='0',
                                                          end_period='2',
                                                          timeout=200).get_data_frames()
    ...
    updated_player_boxscores.to_csv('../data/advanced_player_boxscores_1H/player_advanced_1H_{}.csv'
                                    .format(season), index=False)
    updated_team_boxscores.to_csv('../data/advanced_team_boxscores_1H/team_advanced_1H_{}.csv'
                                  .format(season), index=False)
...
def update_all_data_1H(season='2020-21'):
    """Combines all the update functions above into one function that updates all my data"""
    print("updating basic team boxscores")
    update_team_gamelogs_1H(season=season)
    print("updating advanced boxscores")
    update_advanced_boxscores_1H(season=season)
    print("updating scoring boxscores")
    update_scoring_boxscores_1H(season=season)

    return None



def update_spread_data_1H(season='2020-21'):
    # Get Spreads
    current_spreads_df = pd.read_csv('../data/spreads_1H.csv')
    ...
    updated_spreads.to_csv('../data/spreads_1H.csv', index=False)
